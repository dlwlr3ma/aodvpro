/* -*-  Mode: C++; c-file-style: "gnu"; indent-tabs-mode:nil; -*- */
 ...
 private:
   ...
   void checkenergy (EnergySourceContainer sources);
   ...
   if (InetSocketAddress::IsMatchingType (senderAddress))
     {
       ...
       oss << " received one packet from " << addr.GetIpv4 () ;//<< " " << energy;
       
       
       //zhao dao jiedian de shengyu nenglaing !!!!!!!!!!!!!!!
 ...
 RoutingExperiment::ReceivePacket (Ptr<Socket> socket )
 {
   ...
   //Ptr<EnergySource> source = (*sources).Get(nodeid);
   //double remainingEnergy = (*source).GetRemainingEnergy();
   //std::cout<< remainingEnergy <<std::endl;
 ...
 void
 RoutingExperiment::checkenergy (ns3::EnergySourceContainer sources)
 { 
   for (int i=0 ; i<=49; i++)
   {
   ...
   Simulator::Schedule (MilliSeconds (1000),&RoutingExperiment::checkenergy,this, sources );
 }
 ...
 void
 RoutingExperiment::DepletionHandler (void)
 {
   m_callbackCount++;
 }
 ...
 //Config::SetDefault ("ns3::OnOffApplication::MaxBytes", StringValue ("1280"));
   ...
   WifiRadioEnergyModel::WifiRadioEnergyDepletionCallback callback = MakeCallback (&RoutingExperiment::DepletionHandler, this);
  ...
  // onoff1.SetAttribute ("MaxBytes",UintegerValue(20480));
//  onoff1.SetAttribute ("PacketSize",StringValue ("ns3::ConstantRandomVariable[Constant=64]"));
   //onoff1.SetAttribute ("Interval",StringValue ("ns3::ConstantRandomVariable[Constant=0.02]"));
...
Simulator::Stop (Seconds (TotalTime));
   ...
   Simulator::Destroy ();
   std::cout << "Actual callback count is " << m_callbackCount << std::endl;
 }
